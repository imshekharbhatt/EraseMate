...
const BG_SWATCHES = [
  { id: 'transparent', label: 'Transparent', class: 'sw-transparent', style: {} },
  { id: 'white',  label: 'White',       class: 'sw-white',       style: { background: '#ffffff', border: '1.5px solid #e8e7f0' } },
  { id: 'light',  label: 'Light grey',  class: '',               style: { background: '#f1f5f9' } },
  { id: 'black',  label: 'Black',       class: '',               style: { background: '#111111' } },
  { id: 'purple', label: 'Purple',      class: '',               style: { background: 'linear-gradient(135deg,#5b3ff8,#8b5cf6)' } },
  { id: 'blue',   label: 'Blue',        class: '',               style: { background: 'linear-gradient(135deg,#1d4ed8,#60a5fa)' } },
  { id: 'green',  label: 'Green',       class: '',               style: { background: 'linear-gradient(135deg,#166534,#22c55e)' } },
  { id: 'orange', label: 'Amber',       class: '',               style: { background: 'linear-gradient(135deg,#92400e,#f59e0b)' } },
  { id: 'red',    label: 'Red',         class: '',               style: { background: 'linear-gradient(135deg,#991b1b,#f87171)' } },
]
...
export default function ResultSection({ model, enhanceEdges }: { model: string; enhanceEdges: boolean }) {
  const { originalUrl, resultBlob, resultUrl, currentBg, setCurrentBg, processingMs, modelUsed } = useImageStore()
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => { if (sliding.current) setDividerPos(e.clientX) }
    const onTouchMove = (e: TouchEvent) => { if (sliding.current && e.touches[0]) setDividerPos(e.touches[0].clientX) }
    const onUp = () => { sliding.current = false }
    ...
    <div className="max-w-[1100px] mx-auto mt-8">
      ...
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        <div className="flex items-center gap-3">
          <span className="text-[16px] font-bold text-text">Background Removed</span>
          ...
          {processingMs > 0 && (
            <span className="text-[11.5px] text-muted2">{(processingMs / 1000).toFixed(1)}s · {modelUsed}</span>
          )}
        </div>
      ...
      <div className="bg-white border border-line rounded-[20px] overflow-hidden shadow-md">
        ...
        <div
          ...
          onClick={(e) => { if (!sliding.current) setDividerPos(e.clientX) }}
        >
          {/* Result pane (checkerboard) */}
          <div className="absolute inset-0 flex items-center justify-center checkerboard">
            <canvas ref={canvasRef} className="max-h-[500px] max-w-full object-contain block pointer-events-none" />
          </div>
          ...
          <div
            ...
            style={{ left: '50%' }}
            onMouseDown={(e) => { sliding.current = true; e.preventDefault() }}
            onTouchStart={() => { sliding.current = true }}
          >
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-white border-2 border-purple shadow-[0_2px_10px_rgba(91,63,248,0.25)] flex items-center justify-center hover:shadow-[0_2px_16px_rgba(91,63,248,0.4)] transition-shadow">
              <svg viewBox="0 0 24 24" fill="none" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4 stroke-purple">
          ...
          {/* Labels */}
          <span className="absolute top-3.5 left-3.5 bg-white/92 border border-line px-3 py-1 rounded-md text-[11px] font-bold tracking-[0.05em] text-text2 uppercase z-[25] backdrop-blur-sm shadow-sm">Original</span>
          <span className="absolute top-3.5 right-3.5 bg-white/92 border border-purple-light px-3 py-1 rounded-md text-[11px] font-bold tracking-[0.05em] text-purple uppercase z-[25] backdrop-blur-sm shadow-sm">Removed</span>
        </div>

        {/* Background picker */}
        <div className="border-t border-line px-6 py-4.5 flex items-center gap-5 bg-white flex-wrap py-[18px]">
          <span className="text-[12px] font-bold tracking-[0.06em] text-muted uppercase whitespace-nowrap min-w-[80px]">Background</span>
