...
export default function UploadSection() {
  ...
  return (
    <section className="bg-bg border-y border-line py-16 px-10">
        ...
        {(showDropZone || showError) && (
          ...
          >
            ...
            <p className="text-[13.5px] text-muted leading-relaxed mb-6">
              Drag and drop any photo, or click to browse.<br />
            ...
            >
              <div className="flex items-center gap-2">
                <label className="text-[12px] font-semibold text-muted2 uppercase tracking-wider">Model</label>
                <select
                  value={model}
      ...
      {/* New image button */}
      {showResult && <NewImageButton model={model} enhanceEdges={enhanceEdges} />}
    </section>
  )
}
  ...
  return (
    <div className="bg-white border border-red-200 rounded-[28px] p-14 text-center shadow-md mt-4">
      <div className="w-14 h-14 rounded-xl bg-red-50 flex items-center justify-center mx-auto mb-5">
        <svg viewBox="0 0 24 24" fill="none" className="w-7 h-7 stroke-red-500" strokeWidth={1.75} strokeLinecap="round">
          <circle cx="12" cy="12" r="9"/>
          <path d="M12 8v4m0 4h.01"/>
        </svg>
      </div>
      <h3 className="text-[17px] font-bold text-text mb-2">Processing failed</h3>
      <p className="text-[13.5px] text-muted mb-6">{errorMessage || 'Something went wrong. Please try again.'}</p>
      <button
...
function NewImageButton({ model, enhanceEdges }: { model: string; enhanceEdges: boolean }) {
  const { processFile } = useBackgroundRemoval()
