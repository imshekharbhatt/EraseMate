...
# ─── Supabase Storage ─────────────────────────────────────────────────────────
async def upload_to_supabase_storage(data: bytes, path: str, content_type: str = "image/png") -> Optional[str]:
...
def process_image(raw: bytes, model: str = "auto", enhance_edges: bool = True, bg_color: Optional[str] = None) -> tuple[bytes, dict]:
...
async def remove_background(
    ...
    if is_authenticated:
        ...
        if user_plan == "free" and usage >= FREE_LIMIT:
            raise HTTPException(429, f"Free tier limit reached ({usage}/{FREE_LIMIT} today). Upgrade for unlimited processing.")

    try:
        output_bytes, metadata = process_image(raw, model=model, enhance_edges=enhance_edges, bg_color=bg_color or None)
    ...
    if store and is_authenticated:
        ...
        background_tasks.add_task(upload_to_supabase_storage, output_bytes, storage_path, content_type)
        result_url = f"{SUPABASE_URL}/storage/v1/object/public/{STORAGE_BUCKET}/{storage_path}"
    ...
    resp_headers = {
        ...
        "Access-Control-Expose-Headers": "X-Processing-Time-Ms,X-Model-Used,X-Original-Size,X-Output-Bytes,X-Result-URL",
    }
    if result_url:
